...
{\LARGE\bfseries\color{navy}
  The Hexagon\\[0.2em]
  \large Six Behavioural Profiles for an Italian Fashion Retailer}\\[0.5em]
{\large\color{navy!70}
  Customer Segmentation via UMAP + Gaussian Mixture Models}\\[0.4em]
{\color{navy}\rule{\linewidth}{0.8pt}}\\[1.6em]

{\normalsize
  \textit{Data Science in Action}\quad|\quad LUISS Guido Carli\\[0.3em]
  In partnership with \textbf{Jakala S.p.A.} --- JScience Advanced Analytics\\[0.4em]
  \textbf{Team:}~\textit{The Hexagon}\\[1.2em]
  \begin{tabular}{lll}
    \textbf{$\langle$Member~1 --- delegate$\rangle^{*}$} & \textit{$\langle$matricola$\rangle$} & \texttt{$\langle$email@studenti.luiss.it$\rangle$} \\[2pt]
    \textbf{$\langle$Member~2$\rangle$} & \textit{$\langle$matricola$\rangle$} & \texttt{$\langle$email@studenti.luiss.it$\rangle$} \\[2pt]
    \textbf{$\langle$Member~3$\rangle$} & \textit{$\langle$matricola$\rangle$} & \texttt{$\langle$email@studenti.luiss.it$\rangle$} \\[2pt]
    \textbf{$\langle$Member~4$\rangle$} & \textit{$\langle$matricola$\rangle$} & \texttt{$\langle$email@studenti.luiss.it$\rangle$} \\
  \end{tabular}\\[0.4em]
  {\footnotesize $^{*}$Group delegate (per project rules \S3.1)}\\[1.4em]
  Academic Year 2025--2026\quad$\cdot$\quad May 2026
}\\[1.8em]
...
The analysis follows the \textbf{CRISP-DM} six-phase framework \cite{chapman2000crispdm}:
...
artifacts. \textbf{RobustScaler} (centre by median, scale by IQR; cf.\ Huber's
robust statistics framework \cite{huber1964robust}) is preferred over
...
UMAP \cite{mcinnes2018umap} & Riemannian geometry + algebraic topology; preserves local and global structure & Minor stochastic variation across hardware; recent critiques of UMAP for inferential use \cite{chari2023umap} addressed by treating it as a clustering pre-processor only & \textbf{Selected} \\
\bottomrule
\end{tabular}
\end{table}
...
(i)~\textit{BIC second-difference inflection} \cite{schwarz1978bic}: BIC declines
monotonically as
...
(iii)~\textit{Partition stability via Adjusted Rand Index} \cite{hubert1985ari}.
Stability of the $K{=}6$
...
solution. A Silhouette Score of 0.419 \cite{rousseeuw1987silhouette} falls in the
\textit{good} range; values above 0.5 are achievable only when cluster boundaries
are sharp, which is structurally precluded in continuous behavioural distributions.
...
(McKinsey 2021 \cite{mckinsey2021personalisation}; \textbf{range: 2--8\,pp}).
...
\bibitem{wedel2000market}
  ...
  Kluwer Academic.
\bibitem{chari2023umap}
  Chari, T.\ \& Pachter, L. (2023). The specious art of single-cell genomics.
  \textit{PLOS Computational Biology}, 19(8), e1011288.
\bibitem{huber1964robust}
  Huber, P.J. (1964). Robust estimation of a location parameter.
  \textit{The Annals of Mathematical Statistics}, 35(1), 73--101.
\bibitem{chapman2000crispdm}
  Chapman, P., Clinton, J., Kerber, R.\ et al.\ (2000).
  CRISP-DM 1.0: Step-by-step Data Mining Guide. SPSS Inc./The CRISP-DM Consortium.
...
\bibitem{mckinsey2021personalisation}
  McKinsey \& Company (2021). \textit{The Value of Getting Personalisation Right
  --- or Wrong --- Is Multiplying.} McKinsey Quarterly.
\end{thebibliography}
}
...
\midrule
$\langle$Member~1 --- delegate$\rangle$ &
  Conceptualisation; Methodology; Software (\texttt{src/clustering\_model.py},
  K-sweep validation cells~23--24); Formal analysis;
  Writing --- original draft \\[3pt]
$\langle$Member~2$\rangle$ &
  Software (\texttt{src/data\_processing.py} feature engineering);
  Visualisation (UMAP space, KPI heatmap, geo VIP);
  Writing --- review \& editing \\[3pt]
$\langle$Member~3$\rangle$ &
  Data Curation; Validation (anomaly detection, Pareto/EDA cells);
  Visualisation (Pareto, email funnel, seasonality);
  Writing --- review \& editing \\[3pt]
$\langle$Member~4$\rangle$ &
  Project administration; Investigation (CLV / ROI / CEO P\&L cells~32--36);
  Visualisation (financial dashboards);
...
Generative AI tools (Claude Opus 4.x, ChatGPT-4o) were used in this project for:
(i)~code review, debugging, and refactoring of Python helper modules in
\texttt{src/} and notebook cells; (ii)~drafting and copy-editing assistance on
written text in this report and the accompanying presentations; (iii)~suggesting
the validation extras (bootstrap Silhouette, neighbour ARI, posterior confidence
sweep) implemented in notebook cells~23--24. All analytical decisions, quantitative
results, model choices (UMAP hyperparameters, $K{=}6$ selection), persona
definitions, and business interpretations are the responsibility of the authors;
all numerical figures appearing in this report were produced by executing
\texttt{master\_segmentation.ipynb} directly on the original dataset provided by
Jakala S.p.A., not generated by any AI tool. AI-suggested code and prose were
reviewed, tested against the dataset, and revised by team members before inclusion,
in accordance with project rules \S7.
